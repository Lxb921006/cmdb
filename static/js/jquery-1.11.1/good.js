...
var Table = function() {
    ...
    },
    {
        ...
        value: function tableHtml() {
            ...
            _container.push(['<thead><tr>'].concat(_toConsumableArray(columns.map(function(c) {
                return '<th style="border-bottom:1px solid #e1e1e8;border-left: 1px solid #e1e1e8; padding: 5px 10px;text-align: center"><strong>' + c['title'] + '</strong></th>';
            })), ['</tr></thead>', '<tbody></tbody>']).join(''));
            var container = ['<div class="Table">', '<div class="T-title"style="overflow: auto;">', '<div>' + title + '</div>', '</div>', '<div class="T-content">', '<label for="T-sort">', '<select name="" id="T-pageNum">'].concat(_toConsumableArray([10, 20, 30, 40, 50].map(function(c) {
    ...
    },
    {
        ...
        value: function showPage(data_) {
            ...
            if (data.length > num) {
                ...
                $('#T-page', $ele).find('a.page').on('click',
                function() {
                    var $current = $(this);
    ...
    },
    {
        ...
        value: function events() {
            var that = this;
            var $ele = this.$ele;
            $('#T-pageNum', $ele).on('change',
            function() {
                that.reShow();
                that.showPage();
            });
            $('#T-sort', $ele).on('change',
            function() {
                that.reShow();
                that.showPage();
            });
            $('input[name=T-search]', $ele).on('input propertychange',
            function() {
                that.reShow(1,
                function(data) {
                    that.showPage(data);
